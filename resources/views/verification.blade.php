    ...
    <div class="container-fluid">
        <div class="row">
            <div class="bg-grey1 col-md-4 col-lg-4 col-sm-4 col-xs-12">
                ...
                <form class="mt-5" method="post" action="{{ route('driver.update', $id) }}">
                    {{ csrf_field() }}
                    {{ method_field('PATCH') }}
                    <div class="form-group row pt-3 pb-2 m-auto col-md-11 black_intl">
                        <!--<input type="hidden" name="full_number" id="full_number" value="{{ $contact_number }}"/>-->
                        <input type="text" class="form-control height-input" name="contact_number" value="{{ $contact_number }}" id="phone" placeholder="Enter phone number"/>
